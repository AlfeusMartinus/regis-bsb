...
export const RegistrantList: React.FC<RegistrantListProps> = ({
  mode = "transactions",
  fixedEventId,
}) => {
  ...
  const fetchRegistrants = async () => {
    ...
    if (error) {
      ...
      if (mode === "paid") {
        setRegistrants(
          normalizedData.filter((reg) =>
            SUCCESS_STATUSES.includes(reg.status?.toLowerCase()),
          ),
        );
      } else {
  ...
  const fetchRegistrantsNoJoin = async () => {
    ...
    const { data: events } = await supabase
      .from("events")
      .select(
        "id, title, slug, date_time, location, location_detail, location_link",
      );
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(filteredRegistrants.length / pageSize),
  );
  ...
  const totalSuccessTransactions = registrants.filter((reg) =>
    SUCCESS_STATUSES.includes(reg.status?.toLowerCase()),
  ).length;
  const totalPendingTransactions = registrants.filter(
    (reg) => (reg.status?.toLowerCase() || "pending") === "pending",
  ).length;
  ...
  const handleUndoCheckin = async (registrationId: string) => {
    const confirmed = window.confirm(
      "Batalkan check-in peserta ini? Status kehadiran akan dikembalikan ke belum hadir.",
    );
    if (!confirmed) return;
    setManualCheckinId(registrationId);
    try {
  ...
  const sendTicketEmail = async (reg: any) => {
    ...
    if (!response.ok || result?.success === false) {
      ...
      throw new Error(
        result?.message || fallbackMessage || "Gagal kirim email tiket.",
      );
    }
  };
  ...
  const handleBulkResendTicket = async () => {
    ...
    const confirmed = window.confirm(
      `Kirim tiket ke ${targets.length} peserta sesuai filter saat ini?`,
    );
    if (!confirmed) return;
    ...
    alert(
      `Bulk send selesai. Berhasil: ${successCount}, Gagal: ${failedCount}.`,
    );
  };
  ...
  const handleExportCSV = async () => {
    ...
    const csvContent = [
      ...
      ...filteredRegistrants.map((reg) => {
        const row = [
          ...
          `"${reg.session === "session1" ? "Track 1 – Firebase & Gemini AI" : reg.session === "session2" ? "Track 2 – ADK & AI Product Development" : reg.session || "-"}"`,
          reg.amount,
          reg.status,
    ...
    link.setAttribute(
      "download",
      `registrants_export_${new Date().toISOString().split("T")[0]}.csv`,
    );
    ...
    // Audit log
    await logAudit("EXPORT", "registrations", undefined, {
      eventId,
    ...
    ? [
        {
          ...
          value: selectedRegistrant.created_at
        ...
        { label: "Event", value: selectedRegistrant.events?.title || "-" },
        {
          label: "Sesi / Track",
          value:
            selectedRegistrant.session === "session1"
              ? "Track 1 – Firebase & Gemini AI"
        ...
        },
        {
          label: "Status Kerja",
          value:
            selectedRegistrant.is_working === "yes"
              ? "Sudah Bekerja"
              : selectedRegistrant.is_working === "no"
                ? "Belum / Mencari Kerja"
                : "-",
        },
        {
          label: "Amount",
          value: `Rp ${(Number(selectedRegistrant.amount) || 0).toLocaleString()}`,
        },
        { label: "Status Pembayaran", value: selectedRegistrant.status || "-" },
        {
          label: "Status Check-in",
          value: selectedRegistrant.is_attended ? "Checked-in" : "Belum",
        },
        ...
        { label: "Role", value: selectedRegistrant.current_status || "-" },
        {
          ...
          value:
            selectedRegistrant.current_status === "student"
              ? selectedRegistrant.university || "-"
        ...
        },
        {
          ...
          value:
            selectedRegistrant.current_status === "student"
              ...
              : selectedRegistrant.role || "-",
        },
        {
          label: "Info Source",
          value:
            selectedRegistrant.info_source === "Others"
              ? selectedRegistrant.info_source_others
              : selectedRegistrant.info_source || "-",
        },
        {
          label: "Perangkat External",
          value: selectedRegistrant.uses_external_peripherals ? "YES" : "NO",
        },
        { label: "Mouse Brand", value: selectedRegistrant.mouse_brand || "-" },
        {
          label: "Factors",
          value:
            (selectedRegistrant.work_device_factors?.join(", ") || "-") +
            (selectedRegistrant.work_device_factors_others
              ? ` (${selectedRegistrant.work_device_factors_others})`
              : ""),
        },
        {
          label: "Share Data Sponsor",
          value: selectedRegistrant.share_data_sponsor ? "YES" : "NO",
        },
              ...
              paginatedRegistrants.map((reg) => (
                <tr key={reg.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span
                      className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusColor(reg.status)}`}
                    >
                      {reg.status || "Pending"}
                    </span>
                  </td>
                  {mode === "paid" && (
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span
                        className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getCheckinColor(!!reg.is_attended)}`}
                      >
                    ...
                    <div className="flex flex-col gap-2 min-w-[180px]">
                      <button
                        onClick={async () => {
                          ...
                          if (role === "sponsor") {
                            await logAudit("VIEW", "registrations", reg.id, {
          ...
          <tfoot className="bg-gray-50 font-semibold text-gray-900">
            {mode === "transactions" ? (
              <>
                <tr>
                  <td
                    colSpan={fixedEventId ? 2 : 3}
                    className="px-6 py-3 text-right"
                  >
                    Total Amount (All Status):
                  </td>
      ...
      {filteredRegistrants.length > 0 && (
        <div className="mt-4 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-sm text-gray-600">
            ...
            {Math.min(currentPage * pageSize, filteredRegistrants.length)} dari{" "}
            {filteredRegistrants.length} data
          </p>
